...
function Contacts() {
  ...
  return (
    <>
      <div style={{color:"#1d1d29",backgroundColor:'rgb(29, 29, 41)'}} className='flex flex-col -space-y-1 overflow-y-scroll scrollbar-hide h-[87vh] pb-10'>
        {
          chats?.length > 0 ? chats?.map((e) => {
            return (
              <div style={{backgroundColor:'#23243a'}} onClick={() => {
                ...
                <div className='flex items-center gap-x-3 sm:gap-x-1 md:gap-x-3'>
                  ...
                  <div>
                    <h5 style={{color:"white"}} className='text-[13.6px] sm:text-[16px] text-[#2b2e33] font-bold'>{getChatName(e, activeUser)}</h5>
                    <p style={{color:"#7475a8"}} className='text-[13.6px] sm:text-[13.5px] font-medium text-[#56585c] '>  {e.latestMessage?.message.length > 30
                ...
                <div className='flex flex-col items-end gap-y-[8px]'>
                  <p style={{color:"#4f5178"}} className='text-[12.4px] sm:text-[12px]  font-normal text-[#b0b2b3] tracking-wide'>{timeSince(new Date(Date.parse(e.updatedAt) - aDay))}</p>
                </div>
              </div>
            )
          }) : <NoContacts />
